...
export default function SecurityAudit({ token, handleLogout }) {
  ...
  if (!log) {
    return <Typography>Loading Security Audit Logs...</Typography>;
  }

  const logs = log; // Assuming the log structure is an array of log entries directly
    ...
    <Paper style={{ padding: 20 }}>
      ...
      <div style={{ maxHeight: 600, overflow: 'auto' }}>
        <Grid container spacing={2}>
          {logs.length === 0 ? (
            <Grid item xs={12}>
              <Typography>No security audit logs found.</Typography>
            </Grid>
          ) : (
            logs.map((item, index) => (
